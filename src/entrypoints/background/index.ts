...
export default defineBackground(() => {
  ...
  setupPortListener((port) => {
    ...
    waitUntilReady().then(() => {
      try {
        ...
        logger.debug('Failed to post initial state to port', err);
      }
    });
  ...
  onMessage('stopRecording', async () => {
    ...
    if (guideId) generateGuideMetaOnStop(guideId).catch((err) => {
      logger.warn('Failed to generate guide meta on stop', err);
    });
