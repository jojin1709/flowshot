...
export async function injectContentScript(tabId: number): Promise<void> {
  try {
    ...
    try {
      ...
      logger.debug('Failed to inject content script on tab', tabId, err);
    }
  }
}
  ...
  try {
    ...
    for (const tab of tabs) {
      if (tab.id && isInjectableTab(tab)) {
        sendMessageToTab(tab.id, { type: TabMessage.START_CAPTURE, guideId }).catch((err) => {
          logger.debug('Failed to send START_CAPTURE to tab', tab.id, err);
        });
      }
    }
  } catch (err) {
...
export async function broadcastStopCapture(): Promise<void> {
  try {
    ...
    for (const tab of tabs) {
      if (tab.id) {
        sendMessageToTab(tab.id, { type: TabMessage.STOP_CAPTURE }).catch((err) => {
          logger.debug('Failed to send STOP_CAPTURE to tab', tab.id, err);
        });
      }
    }
  } catch (err) {
